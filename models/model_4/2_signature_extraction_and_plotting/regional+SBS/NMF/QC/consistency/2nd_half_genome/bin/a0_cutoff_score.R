...
feature_path = ifelse(interactive(),
                      yes = "/g/strcombio/fsupek_cancer3/malvarez/chromatin_info/protein_binding/DNA_repair/MMR/MSH6_SETD2_H3K36me3_guominli/2_fold_enrichment_vs_input/bedgraphs_fold_enrich/collapse_ranges/MSH6/SRR13314132_sorted_FE.all_chr.collapsed_ranges.bed.gz",
                      no = args[3]) %>% 
  # remove comment
  gsub("( |\t).*", "", .)
...
good_mappability_regions = ifelse(interactive(),
  ...
  filter(seqnames %in% paste0("chr", c(seq(2,22,2), "Y"))) %>% 
