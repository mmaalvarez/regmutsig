...
if(sum(select(reg_table, trinuc_mut)) > 0){ # only do regression if there are not only 0 muts
    ...
    y_tidy = suppressWarnings(bayesglm(formula = "trinuc_mut ~ SBS96 + offset(log(trinucAtRisk))", 
                                       family = poisson(),
                                       data = reg_table)) %>% 
      ## parse output
      tidy %>% 
