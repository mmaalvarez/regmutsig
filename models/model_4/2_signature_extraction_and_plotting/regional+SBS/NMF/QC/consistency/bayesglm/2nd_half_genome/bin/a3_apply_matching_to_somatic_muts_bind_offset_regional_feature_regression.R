...
## if interactive
#sample_somatic_muts_overlap_feature = c(Sys.glob(paste0("../work/*/*/sample_AY9808_REP1_somatic_muts_overlap_", feature_name, ".tsv"))[1],
#                                        Sys.glob(paste0("../work/*/*/sample_MSM0.10_somatic_muts_overlap_", feature_name, ".tsv"))[1],
#                                        Sys.glob(paste0("../work/*/*/sample_MSK0.5_s4_somatic_muts_overlap_", feature_name, ".tsv"))[1])
...
for(sample_i in seq(1, length(sample_somatic_muts_overlap_feature))){
  ...
  if(sum(select(reg_table, trinuc_mut)) > 0){ # again, only do regression if there are not only 0 muts
    ...
    # GLM for Poisson USING BAYESIAN REG.
    cat(sprintf('Running Poisson regression for regional feature %s in sample %s...\n', feature_name, sample_name))
    y_tidy = suppressWarnings(bayesglm(formula = "trinuc_mut ~ bin + offset(log(trinucAtRisk))", 
                                       family = poisson(),
                                       data = reg_table)) %>% 
      ## parse output
      tidy %>% 
