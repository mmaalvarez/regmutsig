...
## lung cancer
LNSCC = read_tsv("/g/strcombio/fsupek_cancer3/malvarez/WGS_tumors/somatic_variation/TCGA_PCAWG_Hartwig_CPTAC_POG_MMRFCOMMPASS/metadata/comb_metadata_final_6datasets__noconsent_samples_removed__hartwig_upd.tsv") %>%
  filter(OriginalType=="Lung_Non-small cell carcinoma") %>%
  select(sample_id)

## melanoma
melanoma = read_tsv("/g/strcombio/fsupek_cancer3/malvarez/WGS_tumors/somatic_variation/TCGA_PCAWG_Hartwig_CPTAC_POG_MMRFCOMMPASS/metadata/comb_metadata_final_6datasets__noconsent_samples_removed__hartwig_upd.tsv") %>%
  filter(OriginalType %in% c("Skin", "MELA-AU", "SKCM", "SKCM_Melanoma")) %>%
  select(sample_id)

### bind all and write out
write_csv(bind_rows(bind_rows(colorectal, LNSCC), melanoma),
